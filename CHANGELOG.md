...
- **`bastion` is a plain registry dependency again**, pinned to 0.2.4:

  ```toml
  bastion = { registry = "forge", version = "0.2.4" }
  ```

  0.1.3 had to pin it by git commit (`36e85e04…`) because a registry
  dependency did not resolve for archive tasks — `forge scroll.serve` failed
  with `Unknown module Router` even though check/build/test passed. Fixed
  upstream in [march#133](https://github.com/march-language/march/pull/133),
  which also makes `forge deps` fetch a registry package's own dependencies,
  so the git rev pin and its explanatory comment are both gone.

  The dependency resolves to the same bastion 0.2.4 as before — this changes
  how it is fetched, not what is built.

### Notes

- Now requires a March toolchain containing march#133 in addition to march#127
  and march#129 — see [Requirements](README.md#requirements). On an older
  toolchain `forge scroll.serve` fails with `Unknown module Router`; 0.1.3 is
  the last release that works without it.
...
[Unreleased]: https://github.com/march-language/scroll/compare/v0.1.4...HEAD
[0.1.4]: https://github.com/march-language/scroll/compare/v0.1.3...v0.1.4
